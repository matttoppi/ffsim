...
describe('LeaguePicker', () => {
  it('lists leagues for a username, preserving names exactly', async () => {
    ...
    render(<LeaguePicker currentId="l1" onReady={() => {}} onCancel={null} />)
  ...
  it('shows the server message when the user is not found', async () => {
    ...
    render(<LeaguePicker currentId={null} onReady={() => {}} onCancel={null} />)
  ...
  it('selects a league, polls the refresh, and reports ready', async () => {
    ...
    fetchMock.mockImplementation((url: string, init?: RequestInit) => {
      ...
      if (init?.method === 'POST')
        return Promise.resolve(jsonResponse({ status: 'running', league_id: 'l2' }, 202))
      polls += 1
    ...
    render(<LeaguePicker currentId={null} onReady={onReady} onCancel={null} />)
    await submitSearch('matt')
  ...
  it('surfaces a failed refresh and returns to the list', async () => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce(jsonResponse({ status: 'running', league_id: 'l1' }, 202))
      .mockResolvedValueOnce(
        jsonResponse({
          league_id: 'l1',
          name: null,
          ready: false,
          refresh: { status: 'failed', league_id: 'l1', error: 'Sleeper timed out' },
        }),
      )
    ...
    await submitSearch('matt')
    await act(async () => {
      screen.getByText('Dynasty - Triton').closest('button')!.click()
    })
    await act(async () => {
      await vi.advanceTimersByTimeAsync(1600)
    })
