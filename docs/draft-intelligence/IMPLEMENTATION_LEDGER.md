...
**State:** Synthetic audit loop in progress; `needs:vona4:hazard1` root evidence is fixed and its conditional-hazard hot path is optimized without changing policy or statistical budgets
**Branch:** `feat/draft-intelligence`  
**Implementation code changed:** Yes
**Primary next action:** Run and audit the first complete 22-draft synthetic batch, then continue the five-cycle investigation-and-fix loop. Do not claim real-outcome lift, `WAIT`, `REACH`, or personalization from synthetic evidence.
