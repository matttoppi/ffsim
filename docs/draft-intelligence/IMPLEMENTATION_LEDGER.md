...
**State:** Offline nested baseline, official ADP ingestion, FantasyPros-first offensive projections, and an uncalibrated Sleeper-ADP choice baseline are complete; the latest 10-team mock completed, and durable live-session telemetry is implemented for future drafts
**Branch:** `feat/draft-intelligence`  
**Implementation code changed:** Yes
**Primary next action:** Restart the local server before the next mock, then verify that draft `pick`, `draft_state`, `recommendation`, and `league_equity` events are queryable after completion. The completed mock `1393825817661812736` predates telemetry and its historical recommendation boards cannot be recovered. Do not claim `WAIT`, `REACH`, or personalization before out-of-sample calibration.
