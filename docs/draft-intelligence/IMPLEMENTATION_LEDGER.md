...
- [x] Make the user's simulated future picks follow projection value over replacement with open-starting-slot awareness instead of market ADP (ADR-016, model tag `vor1`): the Nabers double-charge disappears (11.3% → 18.0%) and the pick-24 board compresses to an honest 22.0%–15.3% spread because good follow-up picks partially repair any near-tier choice.
...
Absolute championship percentages from the live baseline were inflated (observed ~58% for the slot-1 manager at pick 1 of a 12-team mock): rollouts draft the user's future picks greedily by ADP while opponents sample with probability proportional to 1/ADP, so the user systematically out-drafts eleven noisy opponents. A maximum-likelihood grid fit on 286 observed non-user league-mock picks selected softmax temperature 0.11 (mean NLL 2.09 versus 4.15 at the previous fixed 1.0); with the fitted temperature and three coupled season worlds per continuation, the pick-24 board differentiates structurally (leader 20.7%, kicker 12.7%, defense 8.0%) instead of showing noise-ranked ties. The fit is provisional CPU-heavy mock evidence (ADR-015); refit on human drafts before treating absolute equity as calibrated. Paired candidate deltas remain meaningful because every candidate shares the same future policy and coupled randomness. The same greedy future policy makes back-to-back snake-turn candidates grade exactly even (verified live at mock pick 24: root Love takes Rice at 25 and root Rice takes Love at 25, converging to identical rosters and zero championship flips across 50 continuations); the UI labels these "even" and explains the back-to-back assumption rather than showing +0.0% deltas. Preliminary 12-rollout boards carry roughly ±13% per-candidate standard error, so early rank flashes (a QB briefly first at pick 1) are expected sampling noise that the full pass corrects; the board is dimmed until refined. Fixing the absolutes requires refitting opponent dispersion/temperature on human drafts through the existing leakage-safe backtest; do not present absolute title odds as calibrated before that. The user-future-policy question is decided: deterministic value-over-replacement argmax from the model's own projections (ADR-016).
